...
class ReportPdf {
  ...
  ReportPdf({required this.test, required this.cardType}) {
    init();
  }
  ...
  void displayInformation() {
    ...
    canvas.drawParagraph(
        getParagraphText(test["firstName"] + " " + test["lastName"]),
        Offset(colPos, rowPos));
    rowPos += rowHeight;
    canvas.drawParagraph(
        getParagraphText((DateTime.parse(test["dateOfBirth"])).getAgeYears()),
        Offset(colPos, rowPos));
    rowPos += rowHeight;
    canvas.drawParagraph(
        getParagraphText(test["gender"].toString().toUpperCase()),
        Offset(colPos, rowPos));
    rowPos += rowHeight;
    canvas.drawParagraph(getParagraphText("Self"), Offset(colPos, rowPos));
    ...
    colPos += (rowLength + pixelsPerOneCM);
    rowPos = headerHeight + pixelsPerOneCM;
    canvas.drawParagraph(getParagraphText(test["captureTime"].toString()),
        Offset(colPos, rowPos));
    rowPos += rowHeight;
    canvas.drawParagraph(getParagraphText("Urine"), Offset(colPos, rowPos));
    rowPos += rowHeight;
    canvas.drawParagraph(
        getParagraphText((DateTime.parse(test['panels'][cardType]['details']
    ...
    // if (test[cardType] == "CKD") {
    //   test["results"].remove("urine_ds_uricacid");
    ...
    // List panelList = (test["results"] as Map<String, dynamic>).entries.toList();
    // panelList.sort(
    //     (a, b) => (a.value['index'] ?? 0).compareTo((b.value['index'] ?? 0)));

    // if (test["card"]["TYPE"] != "CKD") {
    //   panelList = panelList
    //       .where((element) =>
    ...
    //   }).toList();
    // }
    List panelList = test["panels"][cardType]["details"]["biomarker_details"];
    ...
    for (int i = 0; i < panelList.length; i++) {
      ...
      canvas.drawParagraph(getParagraphText(panelList[i]["display_name"]),
          Offset(paddingLeft + padding, rowPos));
      canvas.drawParagraph(
          getParagraphText(
      ...
      canvas.drawParagraph(
          ...
          Offset(paddingLeft + padding + colWidth + colWidth * 0.7, rowPos));
      canvas.drawParagraph(
          getParagraphText(panelList[i]["reference_range"],
              style: TextStyle(
                color: Colors.grey,
    ...
    rowPos += test[cardType] != "CKD" ? rowHeight * 1.5 : rowHeight * 2.5;
    canvas.drawLine(
        Offset(paddingLeft, rowPos - pixelsPerOneCM / 2),
        Offset(screenWidth - paddingLeft, rowPos - pixelsPerOneCM / 2),
        tablePaint);
  ...
  Future<String> saveImage(Image image) async {
    ...
    var file = io.File('$path/$directoryName/tempReport.png');
    file.writeAsBytesSync(pngBytes!.buffer.asInt8List());
    print(file.path);
    return file.path;
  }

  final directoryName = 'SDK-Neodocs';
