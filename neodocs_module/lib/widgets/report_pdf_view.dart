...
class ReportPdf {
  ...
  void displayInformation() {
    ...
    canvas.drawParagraph(
        getParagraphText(
            (test["sampleDetails"]["dateOfBirth"].toDate() as DateTime)
                .getAgeYears()),
        Offset(colPos, rowPos));
    rowPos += rowHeight;
    canvas.drawParagraph(
    ...
    canvas.drawParagraph(
        getParagraphText(
            "Dr. ${test["doctor"]["name"].toString().replaceAll("Dr.", "").replaceAll("Dr", "").trim()}"),
        Offset(colPos, rowPos));
    ...
    rowPos = headerHeight + pixelsPerOneCM;
    canvas.drawParagraph(
        getParagraphText((test["captureTime"].toDate() as DateTime)
    ...
    rowPos += rowHeight;
    canvas.drawParagraph(
        getParagraphText((test["startTime"].toDate() as DateTime)
            .format("dd/MM/yyyy, hh:mm:ss a")),
        Offset(colPos, rowPos));
    rowPos += rowHeight;
    canvas.drawParagraph(
        getParagraphText((test["result_date"].toDate() as DateTime)
            .format("dd/MM/yyyy, hh:mm:ss a")),
        Offset(colPos, rowPos));
    ...
    for (int i = 0; i < panelList.length; i++) {
      ...
      canvas.drawParagraph(getParagraphText(panelList[i].value["display_name"]),
          Offset(paddingLeft + padding, rowPos));
      canvas.drawParagraph(
          getParagraphText(
              panelList[i].value["display_value"]?.toString() ??
                  panelList[i].value["estimated_value"].toString(),
              style: panelList[i].value["user_value_flag"] == 0
                  ? normalStyle
                  : titleStyle),
          Offset(paddingLeft + padding + colWidth, rowPos));
      canvas.drawParagraph(
          getParagraphText(panelList[i].value["unit"].toString().isEmpty
              ? "-"
              : panelList[i].value["unit"]),
          Offset(paddingLeft + padding + colWidth + colWidth * 0.7, rowPos));
      canvas.drawParagraph(
          getParagraphText(panelList[i].value["reference_range"],
              style: TextStyle(
                color: Colors.grey,
    ...
    rowPos += test["card"]["TYPE"] != "CKD" ? rowHeight * 1.5 : rowHeight * 2.5;
    canvas.drawLine(
        Offset(paddingLeft, rowPos - pixelsPerOneCM / 2),
        Offset(screenWidth - paddingLeft, rowPos - pixelsPerOneCM / 2),
        tablePaint);
